...
# - Extract all parts
#
# Obs.: The complexity of the script stems from the fact that it creates a small
# check if the default EMLs tab within MBOX is valid. As far as I can tell,
# Thuderbird himself takes care of this, but I think it's best not to assume that
# everything is ok.
...
try:
  with open(mboxName, 'r') as mbox:
    for line in mbox:
      ...
      if bCandidate:
        print(test[:20], end=' | ')
        if re.search('^[^ ]+: [^ ].*$', test) or re.search('^\s+[^ ].*$', test):
          cabLines += 1
        ...
        if re.search('^From $', test) or re.search('^From - ... ... .. ..:..:.. ....$', test):
          # print('ini with from')
